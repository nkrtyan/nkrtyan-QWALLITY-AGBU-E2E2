...
class Helper():
        ...
        try:
            if new_window:
                self.test_logger.info(
                    f'Opening page in a new window: {url}'
                )
            ...
            self._error_with_screenshot(
                f'Go to page failed: {e}'
            )
            raise
    ...
    def find_and_click(self, loc, timeout=5):
        try:
            ...
            self.test_logger.info(
                f'Clicked element: {loc}'
            )

        except Exception as e:
            self._error_with_screenshot(
                f'Find and click failed for {loc}: {e}'
            )
            raise
    ...
    def find_and_send_keys(self, loc, inp_text, timeout=5):
        try:
            ...
            self.test_logger.info(
                f'Entered text into element: {loc}'
            )

        except Exception as e:
            self._error_with_screenshot(
                f'Find and send keys failed for {loc}: {e}'
            )
            raise
    ...
    def get_text(self, loc, timeout=5):
        try:
            ...
            self.test_logger.info(
                f'Got text for element {loc}: {element_text}'
            )
    ...
    def get_attribute(self, loc, attribute, timeout=5):
        try:
            ...
            self.test_logger.info(
                f'Got attribute {attribute} for element {loc}: {attribute_value}'
            )
            ...
            self._error_with_screenshot(
                f'Get attribute failed for {loc}: {e}'
            )
            raise
    ...
    def switch_window(self, window_id=0):
        try:
            ...
            self.test_logger.info(
                f'Switched to window: {window_id}'
            )

        except Exception as e:
            self._error_with_screenshot(
                f'Switch window failed: {e}'
            )
            raise
    ...
    def accept_alert(self, timeout=5):
        try:
            ...
            self.test_logger.info(
                f'Accepted alert with text: {alert_text}'
            )
    ...
    def append_text_to_file(self, file_path, text):
        try:
            ...
            self.test_logger.info(
                f'Appended text to file {file_path}: {text}'
            )

        except Exception as e:
            self.test_logger.error(
                f'Append text to file failed for {file_path}: {e}'
            )
            raise