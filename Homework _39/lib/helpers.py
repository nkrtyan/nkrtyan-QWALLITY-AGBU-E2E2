...
class Helper:
    ...
    def scroll_to_element(self, loc, timeout=5):
        try:
            elem = WebDriverWait(self.driver, timeout).until(
                expected_conditions.presence_of_element_located(loc)
            )
            self.driver.execute_script("arguments[0].scrollIntoView(true);", elem)
            self.test_logger.info(f'Scrolled to element: {loc}')
        except Exception as e:
            self._error_with_screenshot(f'Scroll to element failed for {loc}: {e}')
            raise

    def scroll_down(self, pixels=500):
        try:
            self.driver.execute_script(f"window.scrollBy(0, {pixels});")
            self.test_logger.info(f'Scrolled down by {pixels} pixels')
        except Exception as e:
            self._error_with_screenshot(f'Scroll down failed: {e}')
            raise
    ...
    def append_text_to_file(self, file_path, text):
        try:
            with open(file_path, 'a+') as f:
                f.write(text + '\n')
            self.test_logger.info(f'Append text to file {file_path}: {text}')
        except Exception as e:
